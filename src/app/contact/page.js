...
export default function ContactPage() {
  ...
  const [debugMsg, setDebugMsg] = useState(""); // 新增：用于显示真实错误
    ...
    try {
      const response = await fetch("https://lvhbmoklunrsfnnvonsh.supabase.co/rest/v1/inquiries", {
        ...
        headers: {
          ...
          'Prefer': 'return=representation' // 增强兼容性
        },
      ...
      if (response.ok) {
        ...
        setStatus("ERROR");
        setDebugMsg(result.message || JSON.stringify(result));
      }
    } catch (error) {
      setStatus("ERROR");
          ...
          <div className="bg-black p-12 rounded-[40px] shadow-2xl relative min-h-[450px]">
            {status === "SUCCESS" ? (
              <div className="text-center py-20 flex flex-col items-center justify-center">
                ...
                <p className="text-gray-400 font-bold uppercase text-xs">Our team will contact you at coretoneaudio01@163.com.</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {status === "ERROR" && (
                  <div className="mt-4 p-4 bg-red-900/20 border border-red-500/50 rounded-lg">
                    <p className="text-red-500 text-[10px] font-bold uppercase">System Failure Details:</p>
                    <p className="text-red-400 text-[9px] font-mono mt-1 break-all">{debugMsg}</p>
                  </div>
                )}
              </form>
            )}
