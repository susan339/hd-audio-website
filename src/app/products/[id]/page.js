...
export default function ProductDetailPage({ params }) {
  ...
  const db = {
    '21nc152': { 
      ...
      specs: { 'Power': '4000W Peak', 'Fs': '32 Hz', 'Re': '3.3 Ohm', 'Qts': '0.30', 'Vas': '168 L', 'BL': '32.5 T.m', 'Voice Coil': '153mm', 'Sens': '97dB' },
    ...
    '18tbx100': { 
      ...
      specs: { 'Power': '2400W Peak', 'Fs': '30 Hz', 'Re': '5.1 Ohm', 'Qts': '0.31', 'Vas': '256 L', 'BL': '25.5 T.m', 'Voice Coil': '100mm', 'Sens': '98dB' },
    ...
    '15tbx100': { 
      ...
      specs: { 'Power': '2000W Peak', 'Fs': '35 Hz', 'Re': '5.1 Ohm', 'Qts': '0.28', 'Vas': '113 L', 'BL': '22.1 T.m', 'Voice Coil': '100mm', 'Sens': '96dB' },
    ...
    '10nw64': { 
      ...
      specs: { 'Power': '600W Max', 'Fs': '59 Hz', 'Re': '5.2 Ohm', 'Qts': '0.25', 'Vas': '26 L', 'BL': '18.3 T.m', 'Voice Coil': '64mm', 'Sens': '96dB' },
  ...
  const p = db[id];
  if (!p) return <div className="pt-48 text-center bg-[#0A0A0A] min-h-screen text-white font-mono uppercase">Node Error: 404</div>;

  return (
    <div className="pt-32 pb-24 max-w-[1500px] mx-auto px-6 bg-[#0A0A0A] text-white min-h-screen">
      <Link href="/products" className="text-[#A3A3A3] hover:text-[#E11D48] text-[10px] uppercase mb-12 inline-block font-mono tracking-widest">← Back to Catalog</Link>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div className="space-y-6">
          <div className="bg-white border border-[#262626] p-12 aspect-square flex items-center justify-center overflow-hidden"><img src={p.imgs[0]} alt={p.name} className="max-h-full w-auto object-contain" /></div>
          <div className="grid grid-cols-5 gap-2">
            {p.imgs.map((img, i) => (
              <div key={i} className="bg-[#141414] border border-[#262626] p-2 aspect-square flex items-center justify-center overflow-hidden"><img src={img} alt="" className="max-h-full w-auto object-contain grayscale hover:grayscale-0 transition-all cursor-pointer" /></div>
            ))}
          </div>
        </div>
