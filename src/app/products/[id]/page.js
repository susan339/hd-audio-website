...
export default function ProductDetailPage({ params }) {
  ...
  const db = {
    '21nc152': { 
      ...
      specs: { 'Fs': '32 Hz', 'Re': '3.3 Ohm', 'Qes': '0.32', 'Qms': '6.3', 'Qts': '0.30', 'Vas': '168 L', 'Sd': '1680 cm²', 'Mms': '460 g', 'BL': '32.5 T.m', 'Le': '1.5 mH', 'Power': '4000W Peak', 'Sens': '97dB' },
    ...
    '18tbx100': { 
      ...
      specs: { 'Fs': '30 Hz', 'Re': '5.1 Ohm', 'Qes': '0.33', 'Qms': '7.0', 'Qts': '0.31', 'Vas': '256 L', 'Sd': '1210 cm²', 'Mms': '209 g', 'BL': '25.5 T.m', 'Power': '2400W Peak', 'Sens': '98dB' },
    ...
    '15tbx100': { 
      ...
      specs: { 'Fs': '35 Hz', 'Re': '5.1 Ohm', 'Qes': '0.30', 'Qms': '5.2', 'Qts': '0.28', 'Vas': '113 L', 'Sd': '855 cm²', 'Mms': '163 g', 'BL': '22.1 T.m', 'Power': '2000W Peak', 'Sens': '96dB' },
    ...
    '10nw64': { 
      ...
      specs: { 'Fs': '59 Hz', 'Re': '5.2 Ohm', 'Qes': '0.27', 'Qms': '4.3', 'Qts': '0.25', 'Vas': '26 L', 'Sd': '320 cm²', 'Mms': '47 g', 'BL': '18.3 T.m', 'Power': '600W Max', 'Sens': '96dB' },
    ...
    '12ndl76': { 
      ...
      specs: { 'Fs': '50 Hz', 'Re': '5.3 Ohm', 'Qes': '0.21', 'Qms': '4.2', 'Qts': '0.20', 'Vas': '73 L', 'Sd': '522 cm²', 'Mms': '53 g', 'Power': '800W Peak', 'Sens': '100dB' },
  ...
  if (!p) return <div className="pt-48 text-center bg-[#0A0A0A] min-h-screen text-white font-mono uppercase text-xs">Node Access: NULL</div>;

  return (
    <div className="pt-32 pb-24 max-w-[1500px] mx-auto px-6 bg-[#0A0A0A] text-white min-h-screen">
      <Link href="/products" className="text-[#A3A3A3] hover:text-[#E11D48] text-[10px] uppercase mb-12 inline-block font-mono tracking-widest transition-colors">← Back to Catalog</Link>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div className="space-y-6">
          <div className="bg-white border border-[#262626] p-12 aspect-square flex items-center justify-center overflow-hidden rounded-sm"><img src={p.imgs[0]} alt={p.name} className="max-h-full w-auto object-contain" /></div>
          <div className="grid grid-cols-4 gap-2">
            {p.imgs.map((img, i) => (
              <div key={i} className="bg-[#141414] border border-[#262626] p-2 aspect-square flex items-center justify-center overflow-hidden hover:border-[#E11D48] transition-colors"><img src={img} alt="" className="max-h-full w-auto object-contain grayscale hover:grayscale-0 transition-all" /></div>
            ))}
          </div>
        </div>
        <div className="space-y-12">
          <div>
            ...
            <p className="text-[#A3A3A3] text-[11px] font-mono uppercase tracking-widest border-l-2 border-[#E11D48] pl-4 italic">Professional Grade Transducer Assets</p>
          </div>
          <div className="grid grid-cols-2 gap-4 border-t border-[#262626] pt-8">
            {Object.entries(p.specs).map(([key, val]) => (
              <div key={key} className="border-b border-[#262626] pb-4 bg-[#141414]/30 p-4"><p className="text-[9px] text-[#A3A3A3] uppercase mb-1 font-mono tracking-tighter">{key}</p><p className="text-xl font-bold text-white tracking-tight">{val}</p></div>
            ))}
          </div>
