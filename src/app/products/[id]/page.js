...
export default function ProductDetailPage({ params }) {
  ...
  const db = {
    '21nc152': { 
      ...
      datasheet: 'https://sc02.alicdn.com/kf/H59244c55478f482097993793a0e721c3f.jpg' // 参数规格表图
    },
    '18tbx100': { 
    ...
    '10nw64': { 
      ...
      specs: { 'Power': '600W Max', 'Fs': '59 Hz', 'Re': '5.2 Ohm', 'Qts': '0.25', 'Vas': '26 L', 'BL': '18.3 T.m', 'Sens': '96dB' },
      datasheet: 'https://sc02.alicdn.com/kf/Acc5b5848d22c4951a177de359b32edfa2.png' // 品牌规格书
    },
    '12ndl76': { 
  ...
  return (
    <div style={{ backgroundColor: '#0A0A0A', color: 'white', minHeight: '100vh', padding: '128px 24px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', marginTop: '48px' }}>
        <div>
          <div style={{ backgroundColor: 'white', padding: '48px', aspectRatio: '1/1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><img src={p.imgs[0]} alt="" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} /></div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', backgroundColor: '#262626', border: '1px solid #262626' }}>
            {Object.entries(p.specs).map(([k, v]) => (
              <div key={k} style={{ backgroundColor: '#0A0A0A', padding: '20px' }}>
                <p style={{ fontSize: '10px', color: '#A3A3A3', textTransform: 'uppercase' }}>{k}</p>
                <p style={{ fontSize: '18px', fontWeight: 'bold' }}>{v}</p>
              </div>
            ))}
          </div>
          <div style={{ marginTop: '48px', display: 'flex', gap: '24px' }}>
            ...
            <a href={p.datasheet} target="_blank" style={{ flex: 1, border: '1px solid white', color: 'white', textAlign: 'center', padding: '20px', textDecoration: 'none', fontWeight: 'bold' }}>DOWNLOAD Specs (PDF)</a>
          </div>
        </div>
      </div>
    </div>
