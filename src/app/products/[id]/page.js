...
export default function ProductDetailPage({ params }) {
  ...
  const p = db[id];
  if (!p) return <div style={{ backgroundColor: '#0A0A0A', color: 'white', minHeight: '100vh', padding: '100px', textAlign: 'center' }}>Asset Not Found</div>;

  return (
    <div style={{ backgroundColor: '#0A0A0A', color: 'white', minHeight: '100vh', padding: '128px 24px', fontFamily: 'sans-serif' }}>
      <Link href="/products" style={{ color: '#A3A3A3', textDecoration: 'none', fontSize: '12px', letterSpacing: '0.2em' }}>← BACK TO CATALOG</Link>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', marginTop: '48px' }}>
        <div>
          <div style={{ backgroundColor: 'white', padding: '48px', aspectRatio: '1/1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <img src={p.imgs[0]} alt={p.name} style={{ width: '100%', height: 'auto', objectFit: 'contain' }} />
          </div>
          <div style={{ display: 'flex', gap: '16px', marginTop: '24px' }}>
            {p.imgs.map((img, i) => (
              <img key={i} src={img} alt="" style={{ width: '80px', height: '80px', border: '1px solid #262626', opacity: '0.6', objectFit: 'contain', backgroundColor: 'white' }} />
            ))}
          </div>
        </div>
        <div>
          <span style={{ backgroundColor: '#E11D4822', color: '#E11D48', padding: '4px 12px', fontSize: '10px', fontWeight: 'bold', border: '1px solid #E11D4844' }}>{p.cat}</span>
          <h1 style={{ fontSize: '48px', fontWeight: 'bold', margin: '24px 0', textTransform: 'uppercase', letterSpacing: '-0.05em' }}>{p.name}</h1>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', backgroundColor: '#262626', border: '1px solid #262626' }}>
            {Object.entries(p.specs).map(([k, v]) => (
              <div key={k} style={{ backgroundColor: '#0A0A0A', padding: '24px' }}>
                <p style={{ fontSize: '10px', color: '#A3A3A3', textTransform: 'uppercase', marginBottom: '4px' }}>{k}</p>
                <p style={{ fontSize: '20px', fontWeight: 'bold' }}>{v}</p>
              </div>
            ))}
          </div>
          <div style={{ marginTop: '48px', display: 'flex', gap: '24px' }}>
            <a href="https://wa.me/8615521083699" target="_blank" style={{ flex: 1, backgroundColor: '#E11D48', color: 'white', textAlign: 'center', padding: '20px', textDecoration: 'none', fontWeight: 'bold', letterSpacing: '0.1em' }}>INQUIRY NOW</a>
            <a href={p.datasheet} target="_blank" style={{ flex: 1, border: '1px solid white', color: 'white', textAlign: 'center', padding: '20px', textDecoration: 'none', fontWeight: 'bold', letterSpacing: '0.1em' }}>DOWNLOAD PDF SPECS</a>
          </div>
        </div>
      </div>
    </div>
