...
function ProductsContent() {
  ...
  const allProducts = [
    { 
      ...
      desc: ['4000W Peak Power', '152mm (6") Voice Coil', '97dB Sensitivity'], 
  ...
  return (
    <div className="bg-white min-h-screen pt-32 pb-24 px-8">
      <div className="max-w-[1280px] mx-auto text-left">
        <h1 className="text-[60px] font-black leading-[1] tracking-[-3px] mb-8 uppercase text-black">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-24 gap-y-32">
          {displayedProducts.map(card => (
            <div key={card.id} className="bg-black rounded-[40px] h-80 relative flex items-center overflow-visible group cursor-pointer hover:scale-[1.02] transition-all">
              ...
              <div className="flex-1 ml-48 pr-6 text-center">
                <h3 className="text-white text-[24px] font-black uppercase mb-4 tracking-tighter">{card.name}</h3>
                <ul className="text-zinc-500 text-[11px] font-bold tracking-widest space-y-1 mb-8 uppercase">
                  {card.desc.map(d => <li key={d}>• {d}</li>)}
                </ul>
                <a href={`/products/${card.id}`} className="bg-[#2563EB] text-white px-8 py-2 text-[10px] font-black rounded-full uppercase tracking-widest inline-block">DETAILS</a>
              </div>
            </div>
          ))}
        </div>
